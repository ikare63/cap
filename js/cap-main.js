...
function exerciseSeriesHtml(day,index,meta,light){
 ...
 return `<div class="series-visual" onclick="event.stopPropagation()"><div class="series-dots">${rows.map((s,n)=>`<button type="button" class="series-dot ${s.done?'done':''}" onclick="event.stopPropagation();saveSeries('${day}',${index},${n},${!s.done},null)" aria-label="Série ${n+1}"></button>`).join('')}</div><details class="series-details" onclick="event.stopPropagation()"><summary>${done}/${count} séries · détails</summary><div class="series-grid">${rows.map((s,n)=>`<div class="series-item ${s.done?'done':''}"><label onclick="event.stopPropagation()"><input type="checkbox" ${s.done?'checked':''} onclick="event.stopPropagation()" onchange="saveSeries('${day}',${index},${n},this.checked,null)">Série ${n+1}</label><input type="number" min="0" max="100" inputmode="numeric" value="${escapeHtml(String(s.reps??''))}" placeholder="réps" onclick="event.stopPropagation()" onchange="saveSeries('${day}',${index},${n},null,this.value)"></div>`).join('')}</div></details></div>`;
}

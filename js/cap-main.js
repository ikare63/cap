...
const kpiActivity=document.getElementById('kpiActivity');
const kpiActivityGoal=document.getElementById('kpiActivityGoal');
...
function iconSvg(name){
 const icons={
  ...
  history:'<svg viewBox="0 0 24 24"><path d="M12 6a6 6 0 1 1-5.2 3H4V7l2.3 2.3A8 8 0 1 0 12 4v2Z" fill="currentColor"/><path d="M12 8v4l2.8 1.6" stroke="rgba(255,255,255,.75)" stroke-width="1.8" stroke-linecap="round"/></svg>',
  trends:'<svg viewBox="0 0 24 24"><path d="M4 18h16" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M6 15.5 10.2 11.3 13.4 13.2 18 8.5" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"/><circle cx="6" cy="15.5" r="1.4" fill="currentColor"/><circle cx="10.2" cy="11.3" r="1.4" fill="currentColor"/><circle cx="13.4" cy="13.2" r="1.4" fill="currentColor"/><circle cx="18" cy="8.5" r="1.4" fill="currentColor"/></svg>',
  measures:'<svg viewBox="0 0 24 24"><path d="M5 17 17 5l2 2L7 19H5v-2Z" fill="currentColor"/><path d="m14 4 2 2" stroke="rgba(255,255,255,.75)" stroke-width="1.8" stroke-linecap="round"/></svg>',
  summary:'<svg viewBox="0 0 24 24"><rect x="6" y="4" width="12" height="16" rx="2" fill="none" stroke="currentColor" stroke-width="2"/><path d="M9 9h6M9 13h6M9 17h4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>',
  settings:'<svg viewBox="0 0 24 24"><path d="M12 8.2a3.8 3.8 0 1 0 0 7.6 3.8 3.8 0 0 0 0-7.6Zm8.2 4.6-1.7.6c-.1.4-.3.9-.5 1.3l.8 1.6-1.9 1.9-1.6-.8c-.4.2-.8.4-1.3.5l-.6 1.7h-2.7l-.6-1.7c-.5-.1-.9-.3-1.3-.5l-1.6.8-1.9-1.9.8-1.6c-.2-.4-.4-.9-.5-1.3l-1.7-.6v-2.7l1.7-.6c.1-.4.3-.9.5-1.3l-.8-1.6 1.9-1.9 1.6.8c.4-.2.8-.4 1.3-.5l.6-1.7h2.7l.6 1.7c.5.1.9.3 1.3.5l1.6-.8 1.9 1.9-.8 1.6c.2.4.4.9.5 1.3l1.7.6v2.7Z" fill="currentColor"/></svg>',
  train:'<svg viewBox="0 0 24 24"><circle cx="15" cy="4.8" r="2.1" fill="currentColor"/><path d="M7 8.2h10l2.2 2.3" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"/><path d="M12.2 8.6 10 14l-3.2 3.4" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"/><path d="M13 9.2l2.2 4.7 3.4 1" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"/><path d="M9.8 20.2 11 15h3l1.3 5.2" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"/></svg>',
  bed:'<svg viewBox="0 0 24 24"><path d="M3 18V8.5h2.5a3.5 3.5 0 0 1 3.3 2.3H20a1 1 0 0 1 1 1V18" fill="none" stroke="currentColor" stroke-width="2.1" stroke-linecap="round" stroke-linejoin="round"/><path d="M3 13h18" stroke="currentColor" stroke-width="2.1" stroke-linecap="round"/><path d="M7 10.7h3.5a1.8 1.8 0 0 1 1.8 1.8V13H7z" fill="currentColor"/></svg>'
 };
 return icons[name]||icons.today;
}
nav.innerHTML=navItems.map((x,i)=>{const iconKey=x[0]==='today'?todayNavIcon():x[2];return `<button class="navbtn nav-${x[0]} ${i?'':'active'}" data-view="${x[0]}"><span class="nav-icon">${iconSvg(iconKey)}</span><span>${x[1]}</span></button>`}).join('')+`<button class="navbtn nav-theme" id="navThemeBtn" type="button" aria-label="Activer le mode sombre"><span class="nav-icon" id="navThemeIcon"></span><span id="navThemeLabel">Sombre</span></button>`;nav.querySelectorAll('button[data-view]').forEach(b=>b.onclick=()=>showView(b.dataset.view));
...
function themeIconSvg(isDark){
 return isDark
  ? '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="4.2" fill="currentColor"/><path d="M12 2.5v2.2M12 19.3v2.2M2.5 12h2.2M19.3 12h2.2M5.3 5.3l1.6 1.6M17.1 17.1l1.6 1.6M18.7 5.3l-1.6 1.6M6.9 17.1l-1.6 1.6" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>'
  : '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18.7 3.8A9 9 0 1 0 20 19a7.8 7.8 0 0 1-8-11.8 7.8 7.8 0 0 1 6.7-3.4Z" fill="currentColor"/></svg>';
}
if(navThemeBtn)navThemeBtn.onclick=toggleTheme;
...
function heroAssetForPlan(p){
 ...
 if(title.includes('natation'))return 'assets/hero/hero-natation.png';
 if(title.includes('jogging'))return 'assets/hero/hero-jogging.png';
 if(title.includes('halt'))return 'assets/hero/hero-haltere.png';
 if(title.includes('jamb'))return 'assets/hero/hero-jambes.png';
 if(title.includes('pector'))return 'assets/hero/hero-pecs.png';
 if(title.includes('dos'))return 'assets/hero/hero-dos.png';
 if(title.includes('repos'))return 'assets/hero/hero-repos.png';
 if(title.includes('marche')||title.includes('off')||title.includes('libre'))return 'assets/hero/hero-marche.png';
 return 'assets/hero/hero-dos.png';
}
...
function averageDefined(values){const nums=values.map(Number).filter(v=>Number.isFinite(v)&&v>0);return nums.length?Math.round(nums.reduce((a,b)=>a+b,0)/nums.length):0}
function sleepCompositeScore(s){const hoursScore=Math.max(0,Math.min(100,((Number(s?.hours)||0)/8)*100));return averageDefined([hoursScore,s?.quality,s?.physical,s?.mental])}
function todayActivityProgress(day,p){
 const t=state.training?.[day]||{};
 if(p?.rest||p?.free){return {pct:t.completed?100:0,done:t.completed?1:0,total:1,label:t.completed?'Validé':'À valider'}}
 const total=p?.exercises?.length||0;
 if(!total)return {pct:0,done:0,total:0,label:'Aucune activité'};
 const done=t.completed?total:Object.keys(t.checks||{}).filter(k=>t.checks[k]&&Number(k)<total).length;
 return {pct:Math.round(done/total*100),done,total,label:`${done}/${total} exercice${total>1?'s':''}`};
}
...
function setHomeStatRing(id,value,goal,color){
 const ring=document.getElementById(id);
 if(!ring)return;
 const safeGoal=goal||100;
 const pct=Math.max(0,Math.min(100,((Number(value)||0)/safeGoal)*100));
 ring.style.setProperty('--pct',pct);
 ring.style.setProperty('--ring-color',color);
 ring.style.background=`conic-gradient(${color} ${pct}%, var(--line) 0)`;
}
function renderToday(){
 renderYesterdayScore();
 const d=ensureDate(),p=getPlanForDay(dayName()),a=advice();
 if(todayHeroImage){todayHeroImage.src=heroAssetForPlan(p)+'?v=13';todayHeroImage.onerror=()=>{todayHeroImage.src='assets/hero/hero-repos.png?v=16';};}
 ...
 const sleepScore=sleepCompositeScore(d.sleep);
 const activityProgress=todayActivityProgress(dayName(),p);

 kpiActivity.textContent=activityProgress.pct;
 kpiActivityGoal.textContent=activityProgress.label;
 setHomeStatRing('kpiActivityRing',activityProgress.pct,100,'var(--cyan)');

 kpiSleep.textContent=sleepScore?sleepScore:'—';
 const sleepRingColor=sleepScore>=85?'var(--violet)':sleepScore>=70?'#8f6cff':sleepScore>=55?'var(--warn)':'var(--bad)';
 setHomeStatRing('kpiSleepRing',sleepScore,100,sleepRingColor);
 kpiSleepGoal.textContent=sleepHoursValue
   ? `${sleepHoursValue} h · qualité ${Number(d.sleep.quality)||0} %`
   : 'Nuit dernière';
...
function exerciseSeriesHtml(day,index,meta,light){
 ...
 return `<div class="series-visual"><div class="series-dots">${rows.map((s,n)=>`<button type="button" class="series-dot ${s.done?'done':''}" onclick="saveSeries('${day}',${index},${n},${!s.done},null)" aria-label="Série ${n+1}"></button>`).join('')}</div><details class="series-details"><summary>${done}/${count} séries · détails</summary><div class="series-grid">${rows.map((s,n)=>`<div class="series-item ${s.done?'done':''}"><div class="series-check"><input id="series-${day}-${index}-${n}" type="checkbox" ${s.done?'checked':''} onchange="saveSeries('${day}',${index},${n},this.checked,null)"><label for="series-${day}-${index}-${n}">Série ${n+1}</label></div><input type="number" min="0" max="100" inputmode="numeric" value="${escapeHtml(String(s.reps??''))}" placeholder="réps" onchange="saveSeries('${day}',${index},${n},null,this.value)"></div>`).join('')}</div></details></div>`;
}
...
function renderTraining(){
 ...
 if(p.rest){
  ...
  out+=`<div class="exercise-list-v9">`+p.exercises.map((e,i)=>{
   ...
   return `<div class="exercise"><div class="exercise-row ${showLoad?'':'no-charge'}"><div class="exercise-main"><div class="exercise-main-layout"><input class="exercise-master-check" type="checkbox" ${checked?'checked':''} aria-label="Valider ${escapeHtml(e[0])}" onchange="toggleExercise(${i},this.checked)"><img class="exercise-thumb" src="${exerciseAssetByName(e[0])}?v=31" alt="Illustration ${escapeHtml(e[0])}"><div class="exercise-copy"><div class="exercise-title">${e[0]}</div><div class="exercise-meta">${meta}</div>${strength&&count?exerciseSeriesHtml(selectedTraining,i,e[1],light):''}${e[0]==='Planche avant-bras'?plankTimerHtml(selectedTraining,i):''}${progressionTip(selectedTraining,i)}</div></div></div>${showLoad?`<div class="charge-field"><label>Charge</label><input class="charge-input" type="text" inputmode="decimal" size="5" maxlength="5" value="${escapeHtml(String(getExerciseLoad(selectedTraining,i)))}" placeholder="32,5" onchange="saveExerciseLoad('${selectedTraining}',${i},this.value)" onblur="saveExerciseLoad('${selectedTraining}',${i},this.value)"></div>`:''}</div></div>`;
  }).join('')+`</div>`;
...
function renderMeals(){
 ...
 mealList.innerHTML=meals.map(m=>{
   ...
   return `<div class="meal-item mockup-meal">
    ...
    ${Array.isArray(m.components)&&m.components.length?`<div class="meal-components">${m.components.map(c=>`${escapeHtml(c.name)}${c.unit==='g'?` ${round1(c.amount)} g`:''}`).join(' · ')}</div>`:''}
...
function updateThemeButton(){
 ...
 if(navThemeBtn){
  navThemeBtn.setAttribute('aria-label',isDark?'Activer le mode clair':'Activer le mode sombre');
  if(navThemeIcon)navThemeIcon.innerHTML=themeIconSvg(isDark);
  if(navThemeLabel)navThemeLabel.textContent=isDark?'Clair':'Sombre';
 }
}
function toggleTheme(){document.body.classList.toggle('dark');localStorage.setItem('cap-dark',document.body.classList.contains('dark')?'1':'0');updateThemeButton();}
